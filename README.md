...
Any rule can be scoped to only apply on branches matching one or more
glob patterns — useful for generated files that must stay writable on the
branch that regenerates them, but locked everywhere else:

```text
protect build_registry_outputs/** branches="main,release/*" message="Generated by scripts/regen.py"
```

`chwrite apply` re-evaluates which rules are active every time it runs, so
checking out a branch where the condition no longer matches automatically
unprotects the file again (the same `post-checkout` hook that already runs
`chwrite apply --quiet` on every checkout makes this automatic). Detached
`HEAD` is treated as matching (protection stays on) rather than as
unconditionally unprotected. Full semantics, precedence with ad hoc
`chwrite lock`, and per-worktree behavior: [`SPEC.md`](./SPEC.md) section 33.
